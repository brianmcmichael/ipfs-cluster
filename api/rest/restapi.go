...
func (api *API) idHandler(w http.ResponseWriter, r *http.Request) {
	...
	api.sendResponse(w, 0, err, idSerial)
}
...
func (api *API) graphHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := api.rpcClient.Call("",
		...
		struct{}{},
		&graph)
	api.sendResponse(w, 0, err, graph)
}
...
func (api *API) peerListHandler(w http.ResponseWriter, r *http.Request) {
	...
	api.sendResponse(w, 0, err, peersSerial)
}
...
func (api *API) allocationsHandler(w http.ResponseWriter, r *http.Request) {
	...
	api.sendResponse(w, 0, err, outPins)
}
...
func (api *API) statusAllHandler(w http.ResponseWriter, r *http.Request) {
	...
	if local == "true" {
		...
		err := api.rpcClient.Call("",
			...
			struct{}{},
			&pinInfos)
		api.sendResponse(w, 0, err, pinInfosToGlobal(pinInfos))
	} else {
		...
		err := api.rpcClient.Call("",
			...
			struct{}{},
			&pinInfos)
		api.sendResponse(w, 0, err, pinInfos)
	}
}
	...
	if ps := api.parseCidOrError(w, r); ps.Cid != "" {
		if local == "true" {
			...
			err := api.rpcClient.Call("",
				...
				ps,
				&pinInfo)
			api.sendResponse(w, 0, err, pinInfoToGlobal(pinInfo))
		} else {
			...
			err := api.rpcClient.Call("",
				...
				ps,
				&pinInfo)
			api.sendResponse(w, 0, err, pinInfo)
		}
	}
}
...
func (api *API) sendResponse(
	...
	if err != nil {
		if status == 0 || status < 400 { // set a default error status
